...
class PopulationAlgorithm(object):
    ...
    def __init__(self, drone_pos, blocks, goal, debug=True):
    ...
    def _is_block_covered(self, block):
        """Check if a block is covered
        """
        obj_id, x, y, z = block
        if (DroneWorldObjectId.RED, x, y + 1, z) in self.blocks:
            return True
        elif(DroneWorldObjectId.GREEN, x, y + 1, z) in self.blocks:
            return True
        elif(DroneWorldObjectId.BLUE, x, y + 1, z) in self.blocks:
            return True
        elif(DroneWorldObjectId.YELLOW, x, y + 1, z) in self.blocks:
            return True
        elif (DroneWorldObjectId.BLACK, x, y + 1, z) in self.blocks:
            return True
        elif (DroneWorldObjectId.DRONE, x, y + 1, z) in self.blocks:
            return True
        else:
            return False
        ...
        for block in self.goal:
            ...
            while not found:
                if obj_id == DroneWorldObjectId.RED:
                    ...
                    if red_b not in solution and not self._is_block_covered(red_b):
                    ...
                    if green_b not in solution and not self._is_block_covered(green_b):
                    ...
                    if blue_b not in solution and not self._is_block_covered(blue_b):
                    ...
                    if yellow_b not in solution and not self._is_block_covered(yellow_b):
